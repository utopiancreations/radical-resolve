#!/usr/bin/env python3
"""
Radical Resolve — v3 fresh SFT on scrubbed corpus.

Same recipe as v2, but the training data is radical_resolve_v3.jsonl —
the combined v1 + bonus corpus with first-person fabricated-experience
claims removed by scripts/scrub_first_person_for_v3.py.

Expected wall time: ~2.5 hr (8,376 examples vs v2's 14,262, linear scaling).

Run:
    CUDA_VISIBLE_DEVICES=2 python3 train_radical_resolve_e4b_v3.py
"""
...
DATA_FILE = PROJECT_DIR / "data" / "radical_resolve_v3.jsonl"
BASE_MODEL_PATH = Path.home() / "models" / "gemma-4-E4B-it"
OUTPUT_DIR = PROJECT_DIR / "output" / "radical-resolve-gemma-e4b-v3"
...
def main():
    ...
    print("=" * 60)
    print("Radical Resolve v3 — fresh SFT on scrubbed corpus")
    print("=" * 60)
    ...
    if not DATA_FILE.exists():
        sys.exit(f"ERROR: {DATA_FILE} missing — run scrub_first_person_for_v3.py first")
    base = Path(args.base_model)
    if not base.exists():
        sys.exit(f"ERROR: base weights missing at {base}")
    ...
    with open(FINAL_ADAPTER_DIR / "training_metadata.json", "w") as f:
        json.dump({
            "project": "radical-resolve-v3",
            "base_model": str(base),
            "dataset": str(DATA_FILE),
            "scrub_note": "first-person experience claims removed via scrub_first_person_for_v3.py",
    ...
    print("\n" + "=" * 60)
    print("RADICAL RESOLVE v3 TRAINING COMPLETE")
