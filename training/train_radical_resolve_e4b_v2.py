#!/usr/bin/env python3
"""
Radical Resolve — v2 fresh SFT on combined v1 + bonus corpus.

Identical recipe to v1 (train_radical_resolve_e4b.py), but:
  - Dataset: radical_resolve_v2_combined.jsonl (v1 + 49 bonus samples)
  - Output: output/radical-resolve-gemma-e4b-v2/

Run after v1.1 finishes (sequenced via post_v1_pipeline.sh):
    CUDA_VISIBLE_DEVICES=2 python3 train_radical_resolve_e4b_v2.py
"""
...
DATA_FILE = PROJECT_DIR / "data" / "radical_resolve_v2_combined.jsonl"
BASE_MODEL_PATH = Path.home() / "models" / "gemma-4-E4B-it"
OUTPUT_DIR = PROJECT_DIR / "output" / "radical-resolve-gemma-e4b-v2"
...
def main():
    ...
    print("=" * 60)
    print("Radical Resolve v2 — fresh SFT on combined v1 + bonus")
    print("=" * 60)
    ...
    if not DATA_FILE.exists():
        sys.exit(f"ERROR: {DATA_FILE} missing — concat v1 + bonus first")
    ...
    print("\n" + "=" * 60)
    print("RADICAL RESOLVE v2 TRAINING COMPLETE")
